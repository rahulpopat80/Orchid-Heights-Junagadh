...
import ChatSection from './resident/ChatSection';
...
export default function ResidentDashboard({ session, owners, onRefreshOwners, onLogout }: ResidentDashboardProps) {
  ...
  useEffect(() => {
    if (activeSosAlerts.length > 0) {
      ...
      if (displayedPoll.length === 0) {
        stopHighFrequencyAlarm();
      }
    }
  }, [activeSosAlerts, displayedPoll.length]);
              ...
              <div className="space-y-6">
                ...
                {displayedPoll.length > 0 && (
                  <div className="bg-amber-500 text-slate-950 p-4 rounded-2xl flex items-center justify-between font-bold text-xs shadow-sm border border-amber-400">
                    <p>🚪 {displayedPoll.length} visitor(s) are waiting at the main security gate right now!</p>
                    <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-md rounded-[28px] shadow-2xl border border-slate-100 overflow-hidden z-10 flex flex-col max-h-[85vh] animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="p-5 overflow-y-auto space-y-4">
              {activeSosAlerts.length === 0 && displayedPoll.length === 0 && announcements.length === 0 && activeSocietyNotifs.length === 0 ? (
