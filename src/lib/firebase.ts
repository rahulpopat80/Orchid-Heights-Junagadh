...
// Safely gets service account placeholder
function getHardcodedServiceAccount(): { client_email: string; private_key: string; project_id: string } {
  return { client_email: '', private_key: '', project_id: firebaseConfig.projectId };
}
function legacyGetHardcodedServiceAccount(): { client_email: string; private_key: string; project_id: string } {
  try {
...
export async function sendFCMBroadcast(notification: { title: string; body: string; icon?: string; data?: Record<string, string> }): Promise<void> {
  try {
    ...
    console.log(`[FCM Broadcast] Sending broadcast payload to ${allTokens.length} tokens via server proxy...`);
          ...
          message: {
            ...
            webpush: {
              notification: {
                ...
                badge: "https://i.ibb.co/zT5tpcdY/1000296229-1.png",
                vibrate: [200, 100, 200],
                tag: String(notification.data?.id || notification.data?.type || "orchid_notif")
              },
              fcm_options: {
                link: "/?activeTab=resident"
              },
...
export async function sendFCMPushToFlat(
  ...
  try {
    ...
    console.log(`[FCM] Sending push payload to ${tokens.length} device tokens via server proxy:`, tokens);

    // Send to each token individually using the server proxy endpoint
